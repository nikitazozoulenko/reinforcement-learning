...
class MCTS:
    ...
    def get_solution(self):
        if self.root.solution_found == False:
            return

        perms = []
        node = self.root
        while not node.is_terminate_state:
            for child in node.children:
                if child != None:
                    if child.solution_found:
                        perms += [child.prev_a.cpu().numpy()]
                        node = child
                        break
        return perms
...
class Node:
    def __init__(self, network, s, parent, prev_a, prev_r, is_terminate_state, solution_found=False):
        self.network = network
        self.s = s
        self.parent = parent
        self.prev_a = prev_a
    ...
    def simulate(self, a):
        ...
        self.children[a] = Node(s=s_prime, network=self.network, parent=self, prev_a=a, prev_r=r, is_terminate_state=terminate, solution_found=terminate)
        if terminate:
            estimated_return = torch.tensor(r).float().to(device)
            self.children[a].tree_Q.zero_()
            self.children[a].backprop_solution_found()
        else:
    ...
    def backprop_solution_found(self):
        if self.parent != None:
            if self.parent.solution_found == False:
                self.parent.solution_found = True
                self.parent.backprop_solution_found()

...
def cube_to_tensor(s):
    # white_and_yellow = s.cube_array[0:2,:,:,:]
    # w_y_top = white_and_yellow[:, 0, :, :]
    # w_y_mid0 = white_and_yellow[:, 1, 0:1, :]
    # w_y_mid1 = white_and_yellow[:, 1, 2:3, :]
    # w_y_bot = white_and_yellow[:, 2, :, :]
    # blue_and_green = s.cube_array[2:4, :, :, :]
    # b_g_mid0 = white_and_yellow[:, 1, 0:1, :]
    # b_g_mid1 = white_and_yellow[:, 1, 2:3, :]

    # tensor = torch.from_numpy(np.concatenate([w_y_top, w_y_mid0, w_y_mid1, w_y_bot, b_g_mid0, b_g_mid1], axis=1))
